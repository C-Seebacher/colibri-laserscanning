...
SC2_SDK_FUNC int WINAPI PCO_MergeImagesStop();
// Sets a dll global flag to abort MergeImages. The active thread will cancel sampling and return
// with timeout error. This is only for emergency...

SC2_SDK_FUNC int WINAPI PCO_MergeImages(HANDLE ph, WORD wNumAVGImages, WORD wXRes, WORD wYRes,
                                        WORD wDigitalHBin, WORD wDigitalVBin,
                                        WORD wType, WORD wShift, void* pImage);
// Adds up #wNumAVGImages images with the desired digital binning and shifts the result by wShift
// bits to LSB. The function will work only with one camera at a time. To work on another camera
// call PCO_MergeImages with wNumAVGImages=0. This resets the dll internal variables and frees
// allocated memory. Please call with wNumAVGImages=0 after finishing the capture process, too.
// In: HANDLE ph -> Handle to a previously opened camera.
//     WORD wNumAVGImages -> Number of images to sample (All images will be added) (1...1000)
//     WORD wXRes, wYRes -> Resolution of the original (digitally unbinned) image
//     WORD wDigitalHBin -> Digital horizontal binning (# of pixels sampled to a superpixel) (1...20)
//     WORD wDigitalVBin -> Digital vertikal binning (# of pixels sampled to a superpixel) (1...20)
//     WORD wType -> 2: 16bit output 1: 8bit output will be transferred to pImage
//     WORD wShift -> Binary parameter for number of bits to shift the result to LSB (right)
//     void *pImage -> Memory pointer to receive the calculated image (wType must reflect the
//                     correct byte count per pixel)
